...
% If ecg signal not long enough OR abp signal not long enough OR ppg signal
% not long enough
if( (length(ecg1) < N_d) & ((length(abp_f) < N_d & ~isempty(abp_ind)) | (length(ppg_f) < N_d) & ~isempty(ppg_ind)) )
    alarmResult = 0;
    return;
end
...
%% Bradycardia
brady_thresh = 40;
if (length(selected_signal) >= 5 & strcmp(alarm_type, 'Bradycardia'))
    brady_count = 0;
